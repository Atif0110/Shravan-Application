...
def function_routes(app, db, auth):
    ...
    def ai_generate_content():
        """
        Proxies requests to Gemini's generateContent API using the server-side
        GEMINI_API_KEY. Clients (web or mobile) send the same 'contents' body
        they would have sent directly to Google, so no client-side parsing
        logic needs to change -- only the URL they call.
        """
        data = request.get_json() or {}
        contents = data.get("contents")

        if not contents:
            return jsonify({"error": "contents is required", "status": "fail"}), 400

        try:
            result, status_code = generative_ai.generate_content(contents)
