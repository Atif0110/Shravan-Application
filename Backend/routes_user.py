...
def login_required(view_func):
    """Rejects the request unless a user is logged in (session['user_id'] set)."""
    @wraps(view_func)
...
PRIVILEGED_ROLES = {'caretaker', 'ngo'}


def _current_user_role_name():
    user = User.query.filter_by(user_id=session.get('user_id')).first()
    return user.role.name if user and user.role else None


def _can_access_user(target_user_id):
    """True if the logged-in user is viewing/editing their own record, or holds a
    privileged role (caretaker/ngo) that's allowed to manage other users."""
    if session.get('user_id') == target_user_id:
        return True
    return _current_user_role_name() in PRIVILEGED_ROLES
...
def routes_user(app, db):
    ...
    def get_all_users():
        try:
            if _current_user_role_name() not in PRIVILEGED_ROLES:
        ...
        try:
            ...
            for key, value in data.items():
                if key == 'user_id':
                    ...
                    # Only privileged roles may reassign roles; otherwise a user
                    # could PATCH their own record to grant themselves access.
                    if _current_user_role_name() not in PRIVILEGED_ROLES:
