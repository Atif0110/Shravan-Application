...
onMounted(() => {
  ...
  if (SpeechRecognitionClass) {
    ...
    // continuous + interimResults so the mic keeps listening across natural
    // pauses in speech instead of stopping after the first word or two.
    // Chrome fires onresult repeatedly as the person keeps talking; we only
    // treat a result as "final" once the browser itself marks it final,
    // and we wait for a short pause in speech before sending it, so a
    // sentence isn't cut off mid-way.
    ...
    recognition.onend = () => {
      ...
      // Send whatever was captured once listening actually stops (either
      // the person tapped the mic again, or the browser timed out after
      // real silence) rather than firing on every partial result.
    ...
    recognition.onresult = (event: any) => {
      ...
      // Show live progress in the input box so the person can see it's
