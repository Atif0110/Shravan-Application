...
// YouTube search (Add Video modal)
...
async function searchYoutube() {
  if (!youtubeQuery.value.trim()) return;
  ...
  try {
    ...
    youtubeResults.value = data.results;
  } catch (err) {
    youtubeError.value = err.message || 'Could not search YouTube right now.';
  } finally {
    youtubeSearching.value = false;
  }
}

function pickYoutubeVideo(result) {
  newVideo.value.video_url = result.video_url;
  if (!newVideo.value.title) {
    newVideo.value.title = result.title;
  }
  youtubeResults.value = [];
  youtubeQuery.value = '';
}
      ...
      <div class="modal-content" @click.stop>
        ...
        <div class="modal-body">
          ...
          <form @submit.prevent="addVideo">
            ...
            <div class="form-group">
              <label for="youtube-search">Search YouTube</label>
              <div class="youtube-search-row">
                <input
                  ...
                  @keyup.enter.prevent="searchYoutube"
                >
                <button type="button" class="btn-search-youtube" @click="searchYoutube" :disabled="youtubeSearching">
                  {{ youtubeSearching ? 'Searching...' : '🔍 Search' }}
                </button>
              </div>
              <small v-if="youtubeError" class="youtube-error">{{ youtubeError }}</small>

              <div v-if="youtubeResults.length" class="youtube-results">
                <div
                  v-for="result in youtubeResults"
                  :key="result.video_id"
                  class="youtube-result-card"
                  @click="pickYoutubeVideo(result)"
                >
                  <img :src="result.thumbnail_url" :alt="result.title">
                  <div class="youtube-result-title">{{ result.title }}</div>
                  <div class="youtube-result-channel">{{ result.channel_title }}</div>
                </div>
              </div>
            </div>
            ...
            <div class="form-group">
              ...
              <input 
                ...
                required
              >
              <small>Pick a result above, or paste a YouTube/Vimeo URL directly</small>
            </div>
