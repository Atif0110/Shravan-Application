...
const error = ref('');
// Tracks which reminder is mid-request so its button can show a small
// "saving" state instead of letting a person tap it repeatedly before the
// first click has finished.
...
async function fetchReminders() {
  ...
  try {
    ...
    // Map the backend's field names onto what the template expects, and
    // use today_status (persisted in the medicine_logs table) instead of
    // any purely in-memory status -- this is the piece that was missing
    // before, which is why "Taken" reset every time this page reloaded.
    reminders.value = (data.reminders || []).map(r => ({
      id: r.reminder_id,
      name: r.medicine_name,
      dosage: r.dosage || 'As prescribed',
      timing: r.relation_to_meal ? `${r.time_of_day} - ${r.relation_to_meal}` : r.time_of_day,
...
async function updateReminderStatus(id, status) {
  ...
  try {
    const response = await secureFetch(`${auth_store.backend_url}/api/medicine-logs`, {
      ...
      credentials: 'include',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ reminder_id: id, status })
    });
    ...
    // Update locally too so the UI responds immediately rather than
...
<template>
  <div class="reminders-container" :class="{ 'dark': isDarkMode }">
    ...
    <div class="content-section">
      ...
      <div v-if="error" class="empty-state" style="color: #dc2626;">
        <p>{{ error }}</p>
      </div>
          ...
          <div v-for="reminder in todayReminders" :key="reminder.id" class="reminder-card">
            ...
            <div class="reminder-actions">
              <button 
                ...
                title="Mark as Taken"
              >
                {{ updatingId === reminder.id ? 'Saving...' : '✅ Taken' }}
              </button>
            </div>
          </div>
        </div>
